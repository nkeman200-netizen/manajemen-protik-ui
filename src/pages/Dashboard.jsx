...
function StatCard({ icon: Icon, label, value, gradient, iconBg }) {
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl transition-all duration-300 hover:border-white/20 hover:bg-white/[0.08] hover:shadow-2xl">
      {/* Gradient glow */}
    ...
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl">
      ...
      <div className="divide-y divide-white/5">
        {items && items.length > 0 ? (
          ...
          <div className="px-6 py-10 text-center text-sm text-slate-500">{emptyText}</div>
        )}
      </div>
    </div>
  );
      ...
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-4 rounded-2xl border border-red-500/20 bg-red-500/10 px-8 py-6">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-red-400/70">Terjadi kesalahan saat mengambil data dari server.</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <AgendaSection
          ...
          renderItem={(event) => (
            ...
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary-600/15">
                <CalendarCheck className="h-5 w-5 text-primary-400" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-white">{event.name}</p>
                <p className="text-xs text-slate-400">{formatTanggal(event.start_date)}</p>
              </div>
            </div>
          )}
        />
          ...
          renderItem={(meeting) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-400">{formatTanggal(meeting.date)}</p>
              </div>
            </div>
          )}
        />
